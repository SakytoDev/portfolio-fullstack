...
export default function Header({ showModal }) {
  ...
  return (
    <header className='border-b-2 border-gray-600 px-4 py-2.5 sticky-top text-white backdrop-blur'>
      <div className='flex items-center justify-between'>
        ...
        <div className='flex items-center'>
          { account == null ? 
          <button className='px-4 py-2 border-2 rounded-md transition ease-in-out text-gray-500 border-gray-500 hover:text-white hover:bg-gray-500' onClick={showModal}>Login</button>
          : 
          <Dropdown>
            <Dropdown.Toggle as={avatarToggle}/>
          
            <Dropdown.Menu>
              <Dropdown.Item eventKey="profile" disabled>Profile</Dropdown.Item>
              <Dropdown.Divider />
              <Dropdown.Item eventKey="logout"><p className='text-red-700' onClick={() => logoutAccount()}>Logout</p></Dropdown.Item>
            </Dropdown.Menu>
          </Dropdown>
          }
        </div>
